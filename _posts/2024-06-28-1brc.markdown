...
3. [Multithreading Solution (17.96 secs)](#multithreading-solution-1796-secs)
4. [Custom Number Parsing (8.1 seconds)](#custom-number-parsing-81-seconds)
5. [Custom Key Parsing (6.76 seconds)](#custom-key-parsing-676-seconds)
6. [Custom Hash Function (5.85 seconds)](#custom-hash-function-585-seconds)
7. [Unsafe String Parsing (x seconds)](#unsafe-string-parsing-x-seconds)
8. [Conclusion](#conclusion)

## Introduction

On January 1st, 2024, [Gunnar Morling announced](https://www.morling.dev/blog/one-billion-row-challenge/) the 1 Billion Row Challenge (1BRC). The challenge consists of writing a Java program for retrieving temperature measurement values from a text file and calculating the min, mean, and max temperature per weather station. There’s just one caveat: the file contains 1,000,000,000 rows.
...
## Unsafe string parsing (5.16 seconds)
...
![Flamegraph of the custom hash solution](../assets/images/1brc/flamegraph5.png)

If we zoom in, we see that 11% of the samples are calls to `core::str::converts::from_utf8`:

![Zoomed in flamegraph of the custom hash solution](../assets/images/1brc/flamegraph6.png)
