...
## Custom number parsing (8.1 seconds)
...
![Flamegraph of the first multitheading solution](../assets/images/1brc/flamegraph1.png)
...
![Zoomed in flamegraph of the first multitheading solution](../assets/images/1brc/flamegraph2.png)
...
We know that our test file contains all temperatues in one of these two formats:
1. `ab.c` (e.g., 12.5)
1. `b.c`  (e.g., 5.4)
...
> Note that if we want to support other formats, we just need to update the function by adding a new branch to the `match` statement.

After this change, we have improved the execution time from 17.96 seconds to 8.1 seconds. We are getting closer!

**TODO Ricardo commit link**

## Custom key parsing (6.76 seconds)

Let's now generate another frame graph of the current solution to see what can be our next improvement:

![Flamegraph of the second multitheading solution](../assets/images/1brc/flamegraph3.png)
...
![Zoomed in flamegraph of the first multitheading solution](../assets/images/1brc/flamegraph4.png)
...
![Hash map 1](../assets/images/1brc/hashmap1.png)
...
![Hash map 2](../assets/images/1brc/hashmap2.png)
...
![Hash map with string keys](../assets/images/1brc/hashmap3.png)

HashMap with `u64` keys (inlined):

![Hash map with u64 keys](../assets/images/1brc/hashmap4.png)
...
Running this solution now improves the execution time from 8.1 seconds to 6.76 seconds. We still have some work do to!

**TODO Ricardo commit link**

## Custom hash function (5.85 seconds)

In the previous section, we explained how hash maps work. In the diagrams, we saw that retrieving a value for a given key from the hash map requires first hashing the key, then looking up that hash in the hash table, and finally retriving the data for that hash.
...
One incovenience is that `BTreeMap` does not support custom hashers, so we will have to use a `HashMap` instead. This means we will need to sort the values before printing them.
...
## Unsafe string parsing (x seconds)

This is already optimized and similar as the offcial Rust solution, but can we do a final optimization?