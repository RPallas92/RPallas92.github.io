...
He was right. I was using the `split_once` functions, which performs a linear search over the line. This means it scans through the byte slice from the beginning to the end until it finds the separator character. This can clearly be optimized given the file format.
